...
test.describe('Visual testing on desktop browser', 
  ...
  test('full page visual snapshot', async ({ petAdvicePage }) => {
    expect(await petAdvicePage.page.screenshot({ fullPage: true }))
      .toMatchSnapshot('pet-advice-fullpage.png', { maxDiffPixelRatio: 0.02 });
  });

  test('Dog/Cat Advice buttons visual snapshot', async ({ petAdvicePage }) => {
    expect(await petAdvicePage.dogAdviceButton.screenshot())
      .toMatchSnapshot('dog-advice-button.png', { maxDiffPixelRatio: 0.01 });
    expect(await petAdvicePage.catAdviceButton.screenshot())
      .toMatchSnapshot('cat-advice-button.png', { maxDiffPixelRatio: 0.01 });
  });
});
...
test.describe('Responsive Visual Testing on mobile viewport', 
  ...
  test('Android device visual snapshot', async ({ petAdvicePage }) => {
    ...
    await petAdvicePage.gotoPetAdvicePage();
    expect(await petAdvicePage.page.screenshot({ fullPage: true }))
      .toMatchSnapshot('android-pet-advice-fullpage.png', { maxDiffPixelRatio: 0.02 });
  });
  ...
  test('iOS device visual snapshot', async ({ petAdvicePage }) => {
    ...
    await petAdvicePage.gotoPetAdvicePage();
    expect(await petAdvicePage.page.screenshot({ fullPage: true }))
      .toMatchSnapshot('ios-pet-advice-fullpage.png', { maxDiffPixelRatio: 0.02 });
  });
});
