...
function logApiInfo(request: APIRequestContext, testInfo: TestInfo): void {
  ...
  HTTP_METHODS.forEach((method) => {
    ...
    request[method] = async function (url: string, options?: any): Promise<APIResponse> {
      ...
      try {
        ...
        logger.info(`[API #${callNumber}] ${methodUpper} ${url} → ${response.status()} ${response.statusText()}`);

        // Attach success details (non-blocking)
        attachApiCallDetails(testInfo, callNumber, methodUpper, url, requestData, response).catch((err: Error) =>
          logger.error('Failed to attach API call details:', err.message),
        );
        ...
        logger.error(
          `[API #${callNumber}] ${methodUpper} ${url} → FAILED: ${error instanceof Error ? error.message : String(error)}`,
        );

        // Attach error details (non-blocking)
        attachApiCallDetails(testInfo, callNumber, methodUpper, url, requestData, undefined, error).catch(
          (err: Error) => logger.error('Failed to attach API call details:', err.message),
        );
